...
public class DrawChessBoard {
    ...
    public DrawChessBoard(GameData gameData) {
        ...
        this.DARK_SQUARE_COLOR = SET_BG_COLOR_DARK_GREY;
        this.LIGHT_SQUARE_COLOR = SET_BG_COLOR_LIGHT_GREY;
    ...
    public String drawWhitePerspective() {
        ...
        for (int row = 8; row >= 1; --row) {
            boardBuilder.append(SET_TEXT_COLOR_WHITE).append(intToChar(row)).append(" ");
            for (int col = 1; col <= 8; ++col) {
                // black or white square
                boardBuilder.append((row+col) % 2 == 0 ? DARK_SQUARE_COLOR : LIGHT_SQUARE_COLOR);

                ChessPiece piece = board.getPiece(new ChessPosition(row, col));
                if (piece != null) {
                    boardBuilder.append(piece.getTeamColor() == ChessGame.TeamColor.WHITE
                            ? LIGHT_PIECE_COLOR : DARK_PIECE_COLOR);
                    boardBuilder.append(piece);
                } else {
